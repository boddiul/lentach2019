...
States.Volleyball.prototype = {
    ...
    create: function () {
        ...
        this.add.button(pl_left-200,pl_down-140,'volley-left_down_btn', function (){
            if(canControl){
                player.x = pl_left;
                player.y = pl_down;
            }
        ...
        this.add.button(pl_left -130,pl_up-100,'volley-left_up_btn', function (){
            if(canControl){
                player.x = pl_left;
                player.y = pl_up;
            }
        }).moveDown();
        this.add.button(pl_right-130,pl_down-138,'volley-right_down_btn', function (){
            if(canControl){
                player.x = pl_right;
                player.y = pl_down;
            }
        }).moveDown();
        this.add.button(pl_right-130,pl_up-92,'volley-right_up_btn', function (){
            if(canControl){
                player.x = pl_right;
                player.y = pl_up;
            }
    ...
    update: function () {
        if(game.input.activePointer.isDown){ //свайп вверх
            pointerWasDown = true;
            firstPointerX = game.input.activePointer.y;
        }
        if(pointerWasDown&&game.input.activePointer.isUp){
            pointerWasDown = false;
            if(game.input.activePointer.y<firstPointerX){
        ...
        if(canControl&&(!player.animations.currentAnim.isPlaying && player.animations.currentAnim.name=='jump')){ //Если сейчас не в прыжке
